...
const commonHeaders = {
  ...
  subscriberid: 'testingsubscriberid',
};
...
const axiosBaseQuery =
  (): BaseQueryFn<BaseQueryFnType, unknown, unknown> =>
    async (requestOptions: axiosRequestOptions) => {
      try {
        const result: AxiosResponse = await nodeAxiosInstance({
          ...requestOptions,
        });
        console.log('mini result == ', result);
