            ...
            <Card style={{borderRadius:20,marginBottom:12}} variant={'shadow'}>
                <Box>
                    ...
                    <Box>
                        ...
                        <Box style={{flexDirection:'row',paddingLeft:20,width:200, paddingTop: 3}}>
                        ...
                        <Text 
                            ...
                            lineHeight={31.2} 
                            fontWeight={'700'}
                            style={{fontSize:24}}> {balance.toFixed(2)}</Text>
                        </Box>
                    </Box>
                    ...
                    <Box style={{paddingLeft:20, paddingRight: 20, paddingTop:3, paddingBottom: 20}}>
                    <HorizontalCardSeparator w={1}/>
                    {transactions?.length > 0 ? transactions?.slice(0,2).map((transactionItem,i) => {
                        return(
                        <Box key={i} style={{flexDirection:'row',marginTop:5}}>
                            {/* <Text>{JSON.stringify(transactionItem)}</Text> */}
                            <Box style={{justifyContent:'center'}}>{transactionItem.type === 'moneyin' ? <MoneyInIcon width={14} height={14}/> : <MoneyOutIcon width={14} height={14}/>}</Box>
                            <Box style={{justifyContent:'center',flex:1,marginLeft:10}}>
                                <Text variant={'medium12'} color={'black'} fontFamily={'MTNBrighterSans-Medium'} style={{letterSpacing:-0.5}}>{transactionItem.type}</Text>
                                <Text style={{color:'#A5A5A5'}} lineHeight={13} fontSize={12} fontFamily={'MTNBrighterSans-Regular'}>{transactionItem.transactiontime} {transactionItem.type === 'moneyin' ? '• Money In' : ''}</Text>
                            </Box>
                            <Box style={{justifyContent:'center'}}>
                                <Text 
                                    lineHeight={28} 
                                    color={ transactionItem.type === 'moneyin' ? 'green80' : 'red80'} 
                                    fontFamily={'MTNBrighterSans-Bold'} 
                                    fontWeight={'500'}
                                    fontSize={12}
                                >
                                    {transactionItem.type === 'moneyin' ? '+' : '-'}{currency} {transactionItem.amount.toFixed(2)}
                                </Text>
                            </Box>
                        </Box>
                        )
