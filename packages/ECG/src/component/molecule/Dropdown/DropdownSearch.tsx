...
export default forwardRef(function <T>(
  {
  ...
  const { SearchIcon, LocationIcon } = icon;
  ...
  const handleSearchText = (searchTerm: any) => {
    ...
    //@ts-ignore
    if (value && value.label !== searchText) {
      // setSelected(null);
      //@ts-ignore
  ...
  const openDropdown = async (): Promise<void> => {
    ...
    console.log({ clickTag, payment, selectTag });
            ...
            {value?.icon ? (
              ...
              <Box flexDirection={'row'} alignItems={'center'}>
                <TextInput
                  ...
                  placeholder={`${label || 'Search Here'} ${required ? ' *' : ''
                    }`}
                  numberOfLines={1}
